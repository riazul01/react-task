...
import { ModalBContext } from '../context/ModalBContextProvider';
import '../assets/scss/components/modal.scss';

const ModalB = ({setShowModalB, handleShowModalA}) => {
    const [isChecked, setIsChecked] = useState(false);
    const { data, loader, searchText, setSearchText } = useContext(ModalBContext);
    const [searchParams, setSearchParams] = useSearchParams();
    const navigate = useNavigate();

    useEffect(() => {
        setSearchParams('/problem-2/country-contacts/united-states');
    }, []);
    ...
    return ReactDOM.createPortal(
        <div className="modalBox">
            <div className="modalHeader d-flex justify-content-center gap-3">
                <button onClick={handleShowModalA} className="btn btn1 btn-lg" type="button" >All Contacts</button>
                <button className="btn btn2 btn-lg" type="button" >US Contacts</button>
        ...
        document.getElementById('modal-B')
    );
}
